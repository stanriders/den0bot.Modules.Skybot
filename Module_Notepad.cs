...
namespace den0bot.Modules.Skybot
{
	public class Module_Notepad : IModule
	{
		...
		private string Notepad(Message msg)
		{
			if (Regex.IsMatch(msg.Text, @"n(?:ote)?p(?:ad)?\s+(\d+)$"))
			{
				return ProcessNotepadRead(
					int.Parse(Regex.Match(msg.Text, @"n(?:ote)?p(?:ad)?\s+(\d+)$", RegexOptions.IgnoreCase).Groups[1]
						.ToString()),
					msg.Chat.Id);
			}
		...
		{
			string text = "Добавлено " + DateTime.Now.ToShortDateString() + " в " + DateTime.Now.ToLongTimeString() +
			              " пользователем " + user + "\n" + str;
